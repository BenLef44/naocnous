...
import {
  ...
  CheckSquare, Filter, ChevronDown, ChevronUp, SlidersHorizontal,
} from 'lucide-react';
import { STATUT_CONFIG } from './reglementaire/types';
import { SITE_DATA, RESIDENCE_DATA, AGENT_DATA, PRESTATAIRE_DATA } from './reglementaire/dashboardData';
import { TYPE_CONTROLES } from './reglementaire/ReglementaireSidebar';
...
function generateMockEvents(
  tab: PlanningTab, selectedDateFields: string[],
...
function EventBlock({ ev }: { ev: PlanningEvent }) {
  const cfg = ev.statutConfig;
  return (
    <div className={`rounded-md border-l-[3px] px-2 py-1 text-xs mb-0.5 overflow-hidden cursor-pointer hover:brightness-95 transition-all ${cfg.bg}`}
      ...
      <div className="flex items-center justify-between gap-1 mb-0.5">
        ...
        <span className="text-sm leading-none flex-shrink-0" title={ev.criticite}>{CRITICITE_EMOJI[ev.criticite]}</span>
      </div>
      <div className={`font-semibold truncate leading-tight ${cfg.text}`}>{ev.assignee}</div>
      <div className="text-slate-500 truncate leading-tight text-[10px]">{ev.site}</div>
    </div>
  );
}
        ...
        {(Object.entries(CRITICITE_EMOJI) as [string, string][]).map(([key, emoji]) => {
          ...
          return (
            <button key={key} type="button" onClick={() => onToggleCriticite(key)}
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-medium transition-all
                ${active ? 'bg-slate-100 text-slate-700 border-slate-300' : 'bg-white text-slate-400 border-slate-200 opacity-60 hover:opacity-80'}`}>
...
function MonthGrid({ year, month, events, onNavigate }: {
  year: number; month: number; events: PlanningEvent[]; onNavigate: (delta: number) => void;
}) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="grid grid-cols-7 flex-1 overflow-auto" style={{ gridTemplateRows: `repeat(${cells.length / 7}, minmax(110px, 1fr))` }}>
        {cells.map((day, idx) => {
          const isToday   = day !== null && new Date(year, month, day).toDateString() === today.toDateString();
          const dayEvents = day ? (eventsByDay.get(day) ?? []) : [];
          const MAX = 3; const overflow = dayEvents.length - MAX;
          return (
            <div key={idx} className={`border-r border-b border-slate-100 p-1 min-h-0 overflow-hidden ${!day ? 'bg-slate-50/40' : 'bg-white hover:bg-slate-50/40 transition-colors'}`}>
              {day && (
                <>
                  <div className={`text-xs font-medium mb-1 w-6 h-6 flex items-center justify-center rounded-full flex-shrink-0 ${isToday ? 'bg-emerald-500 text-white' : 'text-slate-500'}`}>{day}</div>
                  {dayEvents.slice(0, MAX).map(ev => <EventBlock key={ev.id} ev={ev} />)}
                  {overflow > 0 && <div className="text-[10px] text-slate-400 font-medium pl-1">+{overflow} de plus</div>}
                </>
              )}
            </div>
          );
...
const CTRL_WEEK_HOURS = Array.from({ length: 14 }, (_, i) => i + 7);
const CTRL_HOUR_H     = 44;
const CTRL_LUNCH_S    = 12.5;
const CTRL_LUNCH_E    = 13.5;
const CTRL_AM_SLOTS   = [8, 9, 10, 11, 12];
const CTRL_PM_SLOTS   = [14, 15, 16, 17];
const CTRL_ALL_SLOTS  = [...CTRL_AM_SLOTS, ...CTRL_PM_SLOTS];
function ctrlHashStr(s: string): number {
  let h = 0;
  for (let i = 0; i < s.length; i++) h = (Math.imul(31, h) + s.charCodeAt(i)) | 0;
  return Math.abs(h);
}
function ctrlAssignHour(evId: string): number {
  return CTRL_ALL_SLOTS[ctrlHashStr(evId) % CTRL_ALL_SLOTS.length];
}

function WeekGrid({ year, month, week, events, onNavigate }: {
  year: number; month: number; week: number; events: PlanningEvent[]; onNavigate: (delta: number) => void;
}) {
  const firstDay = new Date(year, month, 1);
  ...
  const totalH = CTRL_WEEK_HOURS.length * CTRL_HOUR_H;
    ...
    <div className="flex flex-col flex-1 min-h-0">
        ...
        {days.map(d => {
          ...
          return (
            <div key={d.toISOString()} className="flex-1 text-center py-2 border-r border-slate-100 last:border-r-0">
              <div className={`text-xs font-semibold uppercase tracking-wide mb-0.5 ${!isCurrentMonth ? 'text-slate-300' : 'text-slate-400'}`}>{DAY_LABELS[(d.getDay() + 6) % 7]}</div>
              <div className={`text-base font-bold w-8 h-8 flex items-center justify-center rounded-full mx-auto ${isToday ? 'bg-emerald-500 text-white' : !isCurrentMonth ? 'text-slate-300' : 'text-slate-700'}`}>{d.getDate()}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex flex-1 overflow-auto">
        <div className="w-12 flex-shrink-0 relative border-r border-slate-100" style={{ height: totalH }}>
          {CTRL_WEEK_HOURS.map(h => (
            <div key={h} className="absolute w-full flex items-start justify-end pr-1.5" style={{ top: (h - 7) * CTRL_HOUR_H, height: CTRL_HOUR_H }}>
              <span className="text-[10px] text-slate-400 mt-0.5 leading-none">{h}h</span>
            </div>
          ))}
          <div className="absolute left-0 right-0 flex items-center justify-center"
            style={{ top: (CTRL_LUNCH_S - 7) * CTRL_HOUR_H, height: (CTRL_LUNCH_E - CTRL_LUNCH_S) * CTRL_HOUR_H }}>
            ...
            <div key={d.toISOString()} className={`flex-1 relative border-r border-slate-100 last:border-r-0 ${!isCurrentMonth ? 'bg-slate-50/30' : ''}`}
              ...
              {CTRL_WEEK_HOURS.map(h => (
                <div key={h} className="absolute left-0 right-0 border-t border-slate-100"
                  style={{ top: (h - 7) * CTRL_HOUR_H, height: CTRL_HOUR_H }} />
              ))}
              ...
              {Array.from(byHour.entries()).map(([h, evs]) => (
                <div key={h} className="absolute left-0.5 right-0.5 space-y-0.5 overflow-hidden"
                  style={{ top: (h - 7) * CTRL_HOUR_H + 2, maxHeight: CTRL_HOUR_H - 4 }}>
...
function YearGrid({ year, events, onNavigate, onMonthClick }: {
  year: number; events: PlanningEvent[]; onNavigate: (delta: number) => void; onMonthClick: (m: number) => void;
}) {
  const today = new Date();
          ...
          {MONTH_NAMES.map((name, mi) => {
            ...
            return (
              <button key={mi} onClick={() => onMonthClick(mi)}
                className={`rounded-xl border p-3 text-left hover:border-emerald-300 hover:shadow-sm transition-all ${isCurrent ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
                <div className={`text-xs font-semibold mb-2 ${isCurrent ? 'text-emerald-700' : 'text-slate-600'}`}>{name}</div>
                {data.total === 0 ? <div className="text-xs text-slate-300">—</div> : (
                  <div className="space-y-1">
                    {Object.entries(data.statuts).slice(0, 3).map(([s, n]) => {
                      const cfg = (STATUT_CONFIG as Record<string, { dot: string; label: string }>)[s] ?? (STATUT_ACTION_CONFIG as Record<string, { dot: string; label: string }>)[s];
                      if (!cfg) return null;
                      return (
        ...
        <table className="w-full border-collapse" style={{ minWidth: `${180 + daysInMonth * 36}px` }}>
          ...
          <tbody>
            {rows.map(([assignee, { isOrg, events: evs }]) => {
              ...
              evs.forEach(ev => { const d = ev.date.getDate(); if (!evByDay.has(d)) evByDay.set(d, []); evByDay.get(d)!.push(ev); });
              return (
...
// ─── Prestataire/agent used ───────────────────────────────────────────────────

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _unusedPresta = PRESTATAIRE_DATA;
...
export default function ControlesPlanning() {
  const today = new Date();
  const [calMode, setCalMode]   = useState<CalendarMode>('scheduler');
  ...
  const allStatuts    = tab === 'actions' ? Object.keys(STATUT_ACTION_CONFIG) : Object.keys(STATUT_CONFIG);
  ...
  const handleTabChange = (t: PlanningTab) => {
    ...
    setActiveStatuts(new Set(t === 'actions' ? Object.keys(STATUT_ACTION_CONFIG) : Object.keys(STATUT_CONFIG)));
  };

  const toggleStatut    = (s: string) => setActiveStatuts(prev => { const n = new Set(prev); n.has(s) ? n.delete(s) : n.add(s); return n; });
  const toggleCriticite = (c: string) => setActiveCriticites(prev => { const n = new Set(prev); n.has(c) ? n.delete(c) : n.add(c); return n; });
  ...
  const rawEvents = useMemo(() =>
    generateMockEvents(tab, selectedDateFields, year, month),
    [tab, selectedDateFields, year, month]
  );

  const yearEvents = useMemo(() => {
    if (calMode !== 'year') return [];
    const evs: PlanningEvent[] = [];
    for (let m = 0; m < 12; m++) evs.push(...generateMockEvents(tab, selectedDateFields, year, m));
    return evs;
  }, [calMode, tab, selectedDateFields, year]);

  const activeEvents = useMemo(() =>
    (calMode === 'year' ? yearEvents : rawEvents).filter(ev =>
      activeStatuts.has(ev.statut) && activeCriticites.has(ev.criticite)
    ),
    [calMode, yearEvents, rawEvents, activeStatuts, activeCriticites]
  );
